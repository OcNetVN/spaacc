...
function InsertSpaFacility(spaID,CommonID,StrValue)
{
   ...
   // if($(".divSpaFacility"+spaID+CommonID).html()=="" && CommonID != "0")
//       {  
...
function XoaType(spaid,cmid)
{
 ...
 function SelectSpaType(spaID){
    ...
    if(sodiv > 0){
        var cmid = $("#DivSpaTypeShow" + spaID + " div" ).attr("commonid");
        var cm_str = $("#DivSpaTypeShow" + spaID + " div" ).html(); 
...
function UpdateSpaType(spaId){
    ...
    if(sodiv > 0){
         cmid=$('td#tdSaType div').attr("commonid");
    }
    
    $.ajax({
                url: getUrspal() + "admin/spautil/updatespatype",
                type: 'POST',
                async : false,
                data: {
                      spaID:spaId,
...
function UpdateSpaFacitily(spaId){
    ...
    $.ajax({
                url: getUrspal() + "admin/spautil/updatespafaclity",
                type: 'POST',
                async : false,
                data: {
                      kind:'SpaFacility',
...
function LoadTienIchValoaiSpa(){
    ...
    if(list.length >0)
    {
            $.ajax({
            type:"POST",
            url:getUrspal() + "admin/spautil/getspainfo",
            dataType:"json",
