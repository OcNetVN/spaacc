			...
			drawFooter: function(date, options) {
				...
				if(options.showInput) {
					$('<span />')
						...
						.appendTo(footer);
                        //alert("Lan dau : " +  $("#txtSelectedDay").val());
                        var NgayHT = $("#txtSelectedDay").val();
                        if (typeof(NgayHT) == "undefined")
                        {
                            NgayHT  = pMethods.formatDate(date, options);
                            $("#txtSelectedDay").val(NgayHT);
                        }
                        var NgayMoi = pMethods.formatDate(date, options);
                        $("#txtSelectedDay").val(pMethods.formatDate(date, options));
                        var arr_Ngay_HT = NgayHT.split('/');
                        var arr_Ngay_Moi = NgayMoi.split('/');
                        var Ngay_Moi = parseInt(arr_Ngay_Moi[0]);
                        var Ngay_HT = parseInt(arr_Ngay_HT[0]);
                        //alert("Ngay moi: " + NgayMoi +  " | Ngay hien tai: " + NgayHT);
                        //alert("Ngay moi: " +Ngay_Moi +  " | Ngay hien tai: " + Ngay_HT);
                        if(  !(arr_Ngay_HT[0] == arr_Ngay_Moi[0] && arr_Ngay_HT[1] != arr_Ngay_Moi[1]) )
                        {
                            if(!(NgayHT == "" ))
                            {
                                ReloadTimeForProduct(pMethods.formatDate(date, options));
                            }
                            //alert("Ngay hien tai: " + NgayHT + " | Ngay moi click: " + NgayMoi);    
                        }
                        //alert("Sau khi chay : " +  $("#txtSelectedDay").val());
