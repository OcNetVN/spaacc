$(document).ready(function() {
    ...
    $("input[name='disccode']").click(function(){
     ...
     $('input:radio[name=SelCardTypeFor123Pay]').click(function() {
        $('input:radio[name=pay-method]:checked').removeAttr("checked");
      $("#01").attr("checked");
    });
    $('input:radio[name=pay-method]').click(function() {
        var idpaymethod = $(this).attr("id");
        if(idpaymethod != "01")
            $('input:radio[name=SelCardTypeFor123Pay]:checked').removeAttr("checked");
    });
});
...
function gotostep3_step2()
{
    ...
    if(Payment_method=="01")
    {
        ...
        var the = $('input:radio[name=SelCardTypeFor123Pay]:checked').val();
        $.ajax({
            type:"POST",
            ...
            data: {
                the: the,
                check_gotoonlinepay : "goto123pay"
                },
            cache:false,
            success:function (data) {
