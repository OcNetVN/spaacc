...
function doUpload(url) {
	...
	for (i=0;i<files.length;i++) {
        ...
        {
            alert("Filename Invalid: " + fileName);
        }
        else
        {
...
function uploadFile(file, index, url) {
	...
	http.upload.addEventListener('progress', function(event) {	
		...
		//Chờ dữ liệu trả về
		if (fileProgress == 100) {
			ProgressBar.style.background = 'url("../img/progressbar.gif")';
		}
	...
	http.onreadystatechange = function(event) {
		...
		if (http.readyState == 4 && http.status == 200) {
			...
			try { //Bẫy lỗi JSON
				...
				ProgressBar.className += ' progress-bar-danger'; //Thêm class Danger
				ProgressBar.innerHTML = 'Error!'; //Thông báo
			}
		}
		http.removeEventListener('progress',event,false); //Bỏ bắt sự kiện
	}
}
