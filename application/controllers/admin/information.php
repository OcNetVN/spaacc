...
class Information extends CI_Controller {
    ...
    public function index()
    {
       ...
       if($this->CheckQuyen(6)){
          $TypeInfo = $this->m_information->getTypeInfo();
          $TypeLange = $this->m_information->getTypeLange();
          $this->load->view($lang.'/admin/new/v_infomation',array('infotype' =>$TypeInfo,'langtype'=>$TypeLange));   
       }
       else{
    ...
    public function uploadfile(){
        ...
        if($ckq == true)
        {
            $req1 = $this->m_information->uploadfile();
            $req = $req1['res'];
        }
        ...
        echo json_encode($arr);
         ...
         public function UploadFile1(){
            ...
            if ($fileError== 1) { //Lỗi vượt dung lượng
                ...
                move_uploaded_file($_FILES['myfile']['tmp_name'], 'resources/html/'.$TM_Con.'/'.$fileName);                
                // insert vao bang links
                $strURL = 'resources/html/'.$TM_Con.'/'.$fileName;
                $this->m_information->UploadHtml($info,$fileName);
                                
                $url= 'resources/html/'.$TM_Con.'/'.$fileName;
                $fileStatus['status'] = 1;
