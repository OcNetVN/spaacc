  ...
  class M_mail extends CI_Model{
    ...
    public function CreateMail()
    {
       ...
       $mail->isSMTP();                                      // Set mailer to use SMTP
        $mail->Host = 'mail.thebooking.vn';  // Specify main and backup SMTP servers
        $mail->SMTPAuth = true;                               // Enable SMTP authentication
        $mail->Username = 'no-reply@thebooking.vn';                 // SMTP username
        $mail->Password = '123456';                           // SMTP password
        //$mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
        $mail->Port = 587;                                    // TCP port to connect to
        $mail->addReplyTo('info@thebooking.vn', 'TheBooking Information');
        $mail->From = 'no-reply@thebooking.vn';
        $mail->FromName = 'TheBooking.vn';
       return $mail;
    }
    
    public function GetMailTemplate($fname)
    {
        $page_html = file_get_contents('resources/template/' . $fname.'.html');
        return $page_html;
    }
    
    public function GetHTML($fname,$lang)
    {
        $page_html = file_get_contents('resources/html/'.$lang.'/' . $fname);
        $html ='resources/html/'.$lang.'/' . $fname;
